...
class ExperimentRunner:
    ...
    def run_experiment(
        ...
        reference_output_override: Optional[str] = None,
    ) -> Dict[str, Any]:
        ...
        for input_row in dataset_inputs:
            for _ in range(trials):
                ...
                reference_output = (
                    reference_output_override.strip()
                    if reference_output_override and reference_output_override.strip()
                    else input_row.get("reference_output")
                )
                expected_label = str(
                ...
                if task_name in TASK_TYPES["classification"]:
                    ...
                    if reference_output:
                        evaluation = self.summarization_evaluator.evaluate(
                    ...
                    if reference_output:
                        evaluation = self.qa_evaluator.evaluate(
                            reference= reference_output,
                            prediction= execution.output_text,
                        )
                    else:
