...
class BenchmarkRepository:
    ...
    def list_dataset_inputs(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        if limit is None:
            return self._fetch_all(
                "SELECT input_id, input_text, expected_label, reference_output FROM dataset_inputs ORDER BY input_id",
                (),
            )
        return self._fetch_all(
            "SELECT input_id, input_text, expected_label, reference_output FROM dataset_inputs ORDER BY input_id LIMIT %s",
            (limit,),
        )

    def get_dataset_input(self, input_id: int) -> Optional[Dict[str, Any]]:
        return self._fetch_one(
            "SELECT input_id, input_text, expected_label, reference_output FROM dataset_inputs WHERE input_id = %s",
            (input_id,),
        )

    def insert_dataset_input(self, input_text: str, expected_label: str = "", reference_output: str | None = None, dataset_name: str = "runtime_inputs") -> int:
        self._ensure_connection()
        assert self.connection is not None
        ...
        cursor.execute(
            """
            INSERT INTO dataset_inputs (dataset_name, input_text, expected_label, reference_output)
            VALUES (%s, %s, %s, %s)
            ...
            """,
            (dataset_name, input_text, expected_label.strip(), reference_output),
        )
