    ...
    results.append(
        {
            ...
            "reference_output": result.get("reference_output",""),#added this
...
if not SHOW_ACCURACY:
    table_df = df.drop(columns=["output_text", "expected_label", "reference_output","run_records", "accuracy"], errors="ignore")
if not SHOW_QUALITY:
        table_df = df.drop(columns=["output_text", "expected_label", "reference_output","run_records", "quality"], errors="ignore")
    ...
    with tab_chat:
        ...
        if outputs.str.strip().eq("").all():
            ...
            for _, row in df.iterrows():
                ...
                expected = str(row.get("expected_label", "") or row.get("reference_output","")).strip()
                if not expected:
                    run_records = row.get("run_records", [])
                    if isinstance(run_records, list) and run_records:
                        expected = str(run_records[-1].get("expected_label", "") or run_records[-1].get("reference_output","")).strip()
                variant = row['variant']
...
with c2:
    st.download_button(
        "Export Report",
        data=df.drop(columns=["output_text", "expected_label","reference_output","run_records"], errors="ignore").to_csv(index=False),
