...
for run in recent_runs:
    ...
    for variant in run_results:
        rows.append({
            ...
            "reference_output": variant.get("reference_output", ""),
        })

df = pd.DataFrame(rows)

st.write("REFERENCE FROM DB:", recent_runs[0].get("reference_output"))
...
if st.button("View Selected Run"):
    ...
    for _, row in selected.iterrows():
        comparison_results.append({
            ...
            "reference_output": str(row["reference_output"]) if pd.notna(row["reference_output"]) else "",
        })
